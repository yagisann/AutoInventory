    ...
    def regist_inventory(self, mode="normal"):
        ...
        try:
            ...
            for code, reagent in self.inv_reagents.items():
                ...
                if all([reagent.is_checked, not reagent.is_inventory_registered, not reagent.is_using]) or all([mode=="all", not reagent.is_using, not reagent.is_inventory_registerd]):
                    current_code = code
